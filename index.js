...
const config = {
    XBOT: '❉XanarchyBot❉', 
    instagram: 'https://instagram.com/sam_gh_1307', 
    nomer: 'wa.me/50248502976',
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭ ESPERA, proceso largo',
				success: '️❬ ✔ ❭ Hecho 🖤',
				error: {
					stick: 'Bueno, hubo un fallo ;(, Intenta de nuevo ^_^',
					Iv: 'Perdón enlace inválido :('
				},
				only: {
					group: '❬❗❭ Solo en grupos ',
					ownerG: '❬❗❭ Solo el propietario ',
					ownerB: '❬❗❭  Solo el propietario ',
					admin: '❬❗❭ Solo administradores ',
					Badmin: '❬❗❭ El bot debe ser administrador '
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["50248502976@s.whatsapp.net"] 
			...
			switch(command) {
				case 'Probablemente':
					...
					const bisa =['Sí','Tal vez','No']
					const keh = bisa[Math.floor(Math.random() * bisa.length)]
					client.sendMessage(from, 'Pregunta : *'+Probablemente+'*\n\nRespuesta : '+ keh, text, { quoted: mek })
					break
           case 'Es':
					apakah = body.slice(1)
					const apa =['Sí','No','Puede ser']
					const kah = apa[Math.floor(Math.random() * apa.length)]
					client.sendMessage(from, 'Pregunta : *'+apakah+'*\n\nRespuesta : '+ kah, text, { quoted: mek })
					break
				case 'Que porcentaje hay':
					rate = body.slice(1)
